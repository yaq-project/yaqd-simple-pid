...
class SimplePid(HasDependents, HasPosition, IsDaemon):
    ...
    def __init__(self, name, config, config_filepath):
        ...
        self._pid = PID(Kp=self._config["kp"], Ki=self._config["ki"], Kd=self._config["kd"],
                       proportional_on_measurement=self._config["proportional_on_measurement"])
        self._sensor_daemon = yaqc.Client(host=self._config["sensor_daemon"]["host"],
                                      port=self._config["sensor_daemon"]["port"])
        self._control_daemon = yaqc.Client(host=self._config["control_daemon"]["host"],
    ...
    async def update_state(self):
        while True:
            self._busy = False
            self._state["position"] = self._sensor_daemon.get_measured()[self._config["sensor_daemon"]["channel"]]
